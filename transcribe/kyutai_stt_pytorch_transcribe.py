#!/usr/bin/env python3
"""
Kyutai STT 2.6B PyTorch transcriber using the file-based approach.
"""
...
class KyutaiSTTPyTorchTranscriber(BaseTranscriber):
    """Kyutai STT PyTorch transcriber using file-based approach."""
    
    def __init__(self, 
                 hf_repo: str = "kyutai/stt-2.6b-en",
                 device: str = None,
                 torch_dtype: str = "bfloat16",
                 results_dir: str = None):
        # Use safe display name for directories
        display_name = f"kyutai-stt-pytorch-{hf_repo.split('/')[-1]}"
    ...
    def _load_model(self):
        ...
        print(f"Loading Kyutai STT model from {self.hf_repo}...")
    ...
    def transcribe_file(self, audio_file: str) -> TranscriptionResult:
        """
        Transcribe a single audio file using Kyutai STT.
        ...
        try:
            ...
            raise Exception(f"Kyutai STT transcription failed: {e}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Kyutai STT PyTorch Transcription")
    parser.add_argument("--audio_dir", help="Directory containing audio files")
    parser.add_argument("--single_file", help="Process single file only")
    parser.add_argument("--hf_repo", default="kyutai/stt-2.6b-en", help="HF repo to load STT model from")
    ...
    try:
        transcriber = KyutaiSTTPyTorchTranscriber(
    ...
    if args.test_wer:
        try:
            print(f"🎤 Testing Kyutai STT PyTorch on: {args.test_wer}")
